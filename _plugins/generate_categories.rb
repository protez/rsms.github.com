...
module Jekyll
  ...
  # The CategoryPage class creates a single category page for the specified category.
  class CategoryPage < Page
    ...
    def initialize(site, base, category_dir, category)
      ...
      self.data['title']       = "Categories &mdash; #{category}"
      self.data['description'] = "Category: #{category}"
    end
    
  end
  
  # The CategoryIndex class creates a single page with a list of all categories
  class CategoryIndex < Page
    ...
    def initialize(site, base, category_dir)
      ...
      self.data['sorted_categories'] = site.categories.keys.map!{|x| x.to_s}.sort!
    end
  ...
  class Site
    ...
    # Creates an instance of CategoryPage for each category page, renders it, and 
    # writes the output to a file.
    #
    #  +category_dir+ is the String path to the category folder.
    #  +category+     is the category currently being processed.
    def write_category_page(category_dir, category)
      page = CategoryPage.new(self, self.source, category_dir, category)
    ...
    def write_category_index
      if self.layouts.key? 'category_index'
        ...
        index = CategoryIndex.new(self, self.source, dir)
    ...
    def write_category_pages
      if self.layouts.key? 'category'
        ...
        self.categories.keys.each do |category|
          self.write_category_page(File.join(dir, "#{category}"), category)
        end
  ...
  class GenerateCategories < Generator
    ...
    def generate(site)
      site.write_category_index
      site.write_category_pages
    end
